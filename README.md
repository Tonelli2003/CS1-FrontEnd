...
> Plataforma de gerenciamento e monitoramento de ativos industriais desenvolvida em **Python + Streamlit** como entregável das Sprints 1 e 2 do Challenge FIAP. A aplicação opera com **zero dependências externas para processamento de dados**, utilizando exclusivamente os built-ins e estruturas nativas do Python. Visualizações interativas são geradas com **Plotly**.
...
**Sprint 2 — Performance:** `obter_telemetria_historica()` agora é decorada com `@st.cache_data`. O Dashboard chamava essa função para cada ativo da tabela a cada rerun; com cache, cada TAG é computada uma única vez por sessão, eliminando reprocessamento desnecessário.
...
- **Paleta única (`_Palette`):** todos os valores hex do sistema vivem em `PALETTE`. Nenhuma página hard-code cor — todas referenciam `PALETTE.critical`, `PALETTE.safe`, etc. Padrão adaptado de `core/palette.py` do projeto PYRO//GUARD.
- **Componentes reutilizáveis:**
  - `kpi_card()` — cartão de KPI com barra lateral colorida por criticidade, delta com sinal colorido e hint. Usado em Dashboard e Monitoramento com parâmetros distintos.
  - `status_card()` — variante especializada para exibir o resultado de `avaliar_saude()` com cor semântica automática.
  - `section_header()` — cabeçalho de seção com barra vertical, substitui `st.subheader()`.
...
A tela de Monitoramento usa **Plotly** (`plotly.graph_objects`) para os gráficos de séries temporais:

- **Duplo eixo Y:** Temperatura (barras) + Vibração (linha) no mesmo frame — correlação visível.
- **Corrente com área preenchida:** destaca acumulado de carga ao longo do turno.
- **Linhas de limite crítico/alerta** (`add_hline`) com anotação — referência visual permanente.
- **Hover unificado** por horário — todos os valores de um instante em um tooltip só.
- **Cor semântica dinâmica:** a cor das barras de temperatura muda com o status do ativo (verde/âmbar/vermelho), usando `PALETTE` como fonte de verdade.
...
├── utils.py                       # Design system: PALETTE, kpi_card, status_card,
│                                  #   section_header, CSS global, sidebar persistente
├── backend/
│   ├── __init__.py
│   └── mock_db.py                 # Persistência simulada: CRUD + telemetria + avaliação de saúde
│                                  #   @st.cache_data em obter_telemetria_historica()
├── frontend/
...
│   └── 3_📈_Monitoramento.py      # Telemetria em tempo real, Plotly, alertas críticos
├── .streamlit/
...
> O `plotly` é utilizado exclusivamente para **renderização visual** — nenhum dado é
> processado ou transformado por ele; os dados chegam como listas Python nativas.
...
|:---|:---|
| **Dashboard de Ativos** | Métricas de inventário por **Planta/Área** · Filtro `selectbox` por planta · Coluna **Status de Saúde** 🟢🟡🔴 · Exportação CSV com colunas `Planta` e `Saúde` |
| **Monitoramento** | **KPI Cards** com barra lateral colorida por criticidade (design system) · **Gráficos Plotly** com duplo eixo Y, linhas de limite e hover unificado · `@st.cache_data` em telemetria · `st.toast` para status Crítico |
...
*Challenge Sprint 1 & 2 · FIAP · 2026 · Stack: Python 3.11+ · Streamlit 1.33+ · Plotly 5.20+ · Zero-Dependency Data Processing*
